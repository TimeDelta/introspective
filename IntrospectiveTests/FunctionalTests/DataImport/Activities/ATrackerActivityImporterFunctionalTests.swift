...
final class ATrackerActivityImporterFunctionalTests: ImporterTest {
	...
	final override func setUp() {
		super.setUp()
		importer = try! injected(ImporterFactory.self).aTrackerActivityImporter() as! ATrackerActivityImporterImpl
	}
...
"""
	...
	private final func noActivitiesExist() throws -> Bool {
		return try injected(Database.self).query(Activity.fetchRequest()).count == 0
	}

	private final func noDefinitionsExist() throws -> Bool {
		return try injected(Database.self).query(ActivityDefinition.fetchRequest()).count == 0
	}
	...
	private final func objectExists(_ object: NSManagedObject) throws -> Bool {
		return !(try injected(Database.self).pull(savedObject: object).isFault)
	}

	private static func inputFor(_ activities: [ActivityInfo]) -> String {
		var input = Me.headerRow
		for activityInfo in activities {
			let startDateText = injected(CalendarUtil.self).string(for: activityInfo.startDate, inFormat: Me.dateFormat)
			let endDateText: String
			if let endDate = activityInfo.endDate {
				endDateText = injected(CalendarUtil.self).string(for: endDate, inFormat: Me.dateFormat)
			} else {
				endDateText = ""
			}
			input += "\n\"\(activityInfo.name)\","
