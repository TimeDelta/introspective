//
//  MinimumSampleGroupCombiner.swift
//  DataIntegration
//
...
public final class MinimumSampleGroupCombiner: SampleGroupCombiner {

	public static let name = "Minimum"
	...
	public final func combine(groups: [(Any, [Sample])], groupedBy groupAttribute: Attribute, combinationAttribute: Attribute) -> [Sample] {
		...
		for (groupValue, samples) in groups {
			...
			if combinationAttribute is DoubleAttribute {
				combinationValue = DependencyInjector.util.numericSampleUtil.min(for: combinationAttribute, over: samples) as Double
			} else if combinationAttribute is IntegerAttribute {
				combinationValue = DependencyInjector.util.numericSampleUtil.min(for: combinationAttribute, over: samples) as Int
			} else if combinationAttribute is DateAttribute {
				combinationValue = DependencyInjector.util.numericSampleUtil.min(for: combinationAttribute, over: samples) as Date
			} else {
