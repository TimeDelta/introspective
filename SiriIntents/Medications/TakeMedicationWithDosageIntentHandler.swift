...
public final class TakeMedicationWithDosageIntentHandler: NSObject, TakeMedicationWithDosageIntentHandling {
	...
	public func resolveMedication(
		for intent: TakeMedicationWithDosageIntent,
	...
	public func provideMedicationOptions(
		for _: TakeMedicationWithDosageIntent,
	...
	public func handle(
		intent: TakeMedicationWithDosageIntent,
		completion: @escaping (TakeMedicationWithDosageIntentResponse) -> Void
	) {
			...
			guard let medication = try injected(MedicationDAO.self).medicationNamed(medicationName) else {
				...
				completion(
					TakeMedicationWithDosageIntentResponse
						.failure(dosage: dosageText, medication: medicationName)
				)
				return
			}
