...
public final class ImporterFactoryImpl: ImporterFactory {
	...
	public final func wellnessMoodImporter() throws -> WellnessMoodImporter {
		let importers = try injected(Database.self).query(WellnessMoodImporterImpl.fetchRequest())
		...
		return try injected(Database.self).pull(savedObject: importer)
	}

	public final func introspectiveMoodImporter() throws -> IntrospectiveMoodImporter {
		let importers = try injected(Database.self).query(IntrospectiveMoodImporterImpl.fetchRequest())
		if !importers.isEmpty {
			return importers[0]
		}
	...
	public final func easyPillMedicationImporter() throws -> EasyPillMedicationImporter {
		let importers = try injected(Database.self).query(EasyPillMedicationImporterImpl.fetchRequest())
		if !importers.isEmpty {
			return importers[0]
		}
	...
	public final func easyPillMedicationDoseImporter() throws -> EasyPillMedicationDoseImporter {
		let importers = try injected(Database.self)
	...
	public final func aTrackerActivityImporter() throws -> ATrackerActivityImporter {
		let importers = try injected(Database.self).query(ATrackerActivityImporterImpl.fetchRequest())
		if !importers.isEmpty {
			return importers[0]
		}
